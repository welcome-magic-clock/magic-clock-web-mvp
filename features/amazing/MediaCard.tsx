  ...
  async function handleFreeAccess() {
    ...
    try {
      const res = await fetch("/api/access/free", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ contentId: item.id, creatorHandle: item.user }),
      });
      // La route retourne { ok: true } ou { decision: "ALLOWED" }
      // On considère tout status 2xx comme un succès
      if (res.ok) {
        setIsUnlocked(true);
        // Redirection vers la page de confirmation FREE
        // (même UX que /payment/result pour PPV et abonnement)
        window.location.href =
