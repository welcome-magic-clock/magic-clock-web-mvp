...
// ✅ v8 — Cœur cliquable sur la carte Amazing (même logique que page détail)
// ✅ liked initialisé depuis Supabase au montage via /api/magic-clocks/[id]/liked
// ✅ Toggle like 1/user · optimistic UI · rollback sur erreur
// ✅ Étoiles depuis rating_avg Supabase (inchangé)
...
export default function MediaCard({ item }: Props) {
  // ── Créateur — 100% depuis FeedCard (Supabase) ──────────────────────────
  ...
  const rawHashtags = Array.isArray((item as any).hashtags)
  ...
  const beforeThumb: string =
    (item as any).beforeThumbnail ??
    (item as any).beforeThumb ??
    item.beforeUrl ??
    item.image ??
    FALLBACK_BEFORE;
  const afterThumb: string =
    (item as any).afterThumbnail ??
    (item as any).afterThumb ??
    item.afterUrl ??
    item.image ??
    FALLBACK_AFTER;
  const heroVideoSrc = isVideo(item.afterUrl)
    ? (item.afterUrl as string)
    : isVideo(item.beforeUrl)
  ...
  // ── État likes — optimistic UI + init depuis Supabase ───────────────────
  const magicClockId = item.magicClockId ?? String(item.id);
  const [likesCount, setLikesCount] = useState(
    typeof (item as any).likes === "number" ? (item as any).likes : 0
  );
  const [liked, setLiked] = useState(false);
  const [likeLoading, setLikeLoading] = useState(false);

  // Init liked depuis Supabase au montage (1 appel léger par carte)
  useEffect(() => {
    fetch(`/api/magic-clocks/${magicClockId}/liked`)
      ...
      .catch(() => {/* silencieux — user non connecté = false */});
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [magicClockId]);

  // ── Toggle like ──────────────────────────────────────────────────────────
  async function handleLike(e: React.MouseEvent) {
    e.preventDefault();   // Éviter la navigation vers la page détail
    e.stopPropagation();
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{
            height: "30%",
            background: "linear-gradient(to top,rgba(10,15,30,.45),transparent)",
          }}
        />
      ...
      <div className="px-3 pt-2.5 pb-3 space-y-1.5">
        {/* Ligne 1 : avatar · nom · handle · vues · ❤️ cliquable · étoiles */}
        <div className="flex items-center gap-1.5 min-w-0">
          ...
          >
            ...
            {isCertified && (
              <BadgeCheck className="h-3 w-3 flex-shrink-0 text-violet-400" />
            )}
          ...
          {/* ❤️ Cœur cliquable — toggle 1 like/user */}
          <button
            type="button"
            onClick={handleLike}
          ...
          >
            <Heart
              ...
              style={
                liked
                  ? { fill: "#F54B8F", color: "#F54B8F", filter: "drop-shadow(0 0 2px #F54B8F88)" }
                  : {}
              }
            />
        ...
        <div className="flex gap-2 pt-0.5">
          <button
            ...
            style={{
              background:
                "linear-gradient(135deg,#4B7BF5,#7B4BF5,#C44BDA,#F54B8F,#F5834B)",
