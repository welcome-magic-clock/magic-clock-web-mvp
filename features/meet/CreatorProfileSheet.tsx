...
// ✅ v2.2 — Pleine page (100dvh) · bottom sheet full height · Lucide icons
"use client";
...
export function CreatorProfileSheet({ creator, isMet, onMeet, onClose }: Props) {
  ...
  return (
    ...
    <div
      ...
      onClick={onClose}
    >
      {/* Sheet — prend toute la hauteur disponible */}
      <div
        className="w-full overflow-y-auto bg-white"
        style={{
          // Pleine hauteur sauf les 40px du safe area top
      ...
      >
        {/* Handle */}
        <div className="mx-auto mt-3 h-1 w-9 rounded-full bg-slate-200 flex-shrink-0" />
        ...
        <div
          ...
          style={{
            height: 72,
            background: "linear-gradient(160deg,rgba(75,123,245,.1) 0%,rgba(196,75,218,.08) 50%,rgba(245,131,75,.05) 100%)",
          }}
        />
        ...
        <div className="px-5 pb-12">
          ...
          <div style={{ marginTop: -44, marginBottom: 12 }}>
            <div className="relative" style={{ width: 80, height: 80 }}>
              ...
              <div
                ...
                style={{ inset: -3, background: haloGrad, animation: "mcHaloSpin 4s linear infinite" }}
              />
              ...
              <div
                ...
                style={{ inset: 3, background: "linear-gradient(135deg,#ede9fe,#dbeafe)", border: "2px solid white" }}
              >
                {creator.avatar
          ...
          <button
            type="button"
            className="absolute right-4 flex h-8 w-8 items-center justify-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200 transition-colors"
            style={{ top: "calc(72px + 20px)" }}
            onClick={onClose}
          >
            <X className="h-4 w-4" />
          </button>
          ...
          {/* Bio */}
          {creator.bio && (
            <p className="mt-3 text-[13px] leading-relaxed text-slate-500">{creator.bio}</p>
          )}
            ...
            {[
              ...
              <div key={lbl} className="rounded-[16px] py-3.5 text-center" style={{ background: "#f8fafc", border: "1px solid #e2e8f0" }}>
                <p
                  ...
                  style={{ background: "linear-gradient(135deg,#4B7BF5 0%,#7B4BF5 30%,#C44BDA 55%,#F54B8F 80%,#F5834B 100%)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}
                >
                  {val}
                </p>
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <span className="flex items-center gap-1 text-[10px] text-slate-400">
              <Globe2 className="h-3 w-3" />{creator.langs.join(" · ")}
            </span>
          </div>
          ...
          <div className="mt-5 flex gap-2">
            ...
            <button
              type="button"
              className="flex h-14 w-14 flex-shrink-0 items-center justify-center rounded-[14px] transition-colors"
          ...
          {followers.length > 0 && (
            <>
              ...
              <div className="mt-3 grid grid-cols-4 gap-3">
                {followers.map((f, i) => (
                  <button key={i} type="button" className="flex flex-col items-center gap-1.5 transition-transform hover:-translate-y-0.5">
                    <div
                      className="flex h-14 w-14 items-center justify-center rounded-full text-[16px] font-black"
                      style={{ background: `${f.color}15`, border: `2px solid ${f.color}30`, color: f.color }}
                    >
                      {f.initial}
                    </div>
            ...
            {["Technique signature", "Collection hiver"].map((title, i) => (
              <div key={title} className="overflow-hidden rounded-[16px]" style={{ border: "1px solid #e2e8f0" }}>
                <div
                  ...
                  style={{ background: i === 0 ? "linear-gradient(135deg,rgba(75,123,245,.07),rgba(123,75,245,.06))" : "linear-gradient(135deg,rgba(196,75,218,.06),rgba(245,75,143,.05))" }}
                >
                ...
                <div className="p-2.5">
                  ...
                  <span
                    className="mt-2 inline-block rounded-[6px] px-1.5 py-0.5 text-[8px] font-extrabold uppercase"
                    style={i === 0
