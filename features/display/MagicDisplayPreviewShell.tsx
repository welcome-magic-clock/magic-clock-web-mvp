...
import { CubeFaceMedia, default as LazyMediaSlot } from "@/components/media/LazyMediaSlot";
...
function getFaceMainPhotoUrl(face: PreviewFace | undefined): string | null {
  if (!face) return null;
  if (face.coverMedia?.url) return face.coverMedia.url;
  const allMedia: PreviewMedia[] =
    face.segments?.flatMap((s) => s.media ?? []) ?? [];
  if (!allMedia.length) return null;
  const photo = allMedia.find((m) => m.type === "photo") ?? allMedia[0];
...
export default function MagicDisplayPreviewShell({
  ...
  const [fullScreenFaceIndex, setFullScreenFaceIndex] = useState<number | null>(null);
  ...
  const [openedSegmentId, setOpenedSegmentId] = useState<string | number | null>(null);

  const safeIndex = !hasFaces ? 0 : Math.min(Math.max(activeFaceIndex, 0), faces.length - 1);
  const activeFace = hasFaces ? faces[safeIndex] : undefined;

  const detailFace = openedFaceForDetails !== null ? faces[openedFaceForDetails] : undefined;
  ...
  const [detailEditorState, setDetailEditorState] = useState<PersistedFaceState | null>(null);
  ...
  useEffect(() => {
    ...
    const faceId = openedFaceForDetails + 1;
  ...
  if (detailEditorState?.segments && detailEditorState.segments.length > 0) {
    const countFromState = detailEditorState.segmentCount ?? detailEditorState.segments.length;
    ...
    editorSegmentsForDetail = detailSegments.map((seg, index) => {
      ...
      return {
        ...
        label: (seg.title as string | undefined)?.trim() || `Segment ${index + 1}`,
        mediaType,
        mediaUrl,
        notes:
  ...
  const editorOppositeSegment =
    oppositeSegmentIdForDetail != null
      ? editorSegmentsForDetail.find((seg) => seg.id === oppositeSegmentIdForDetail) ?? null
      : null;

  const leftHasMedia = !!(editorSelectedSegment && editorSelectedSegment.mediaUrl);
  const rightHasMedia = !!(editorOppositeSegment && editorOppositeSegment.mediaUrl);
  ...
  function goPrevFace() { goToFace(activeFaceIndex - 1); }
  function goNextFace() { goToFace(activeFaceIndex + 1); }
  ...
  function handleCubePointerUp(e: React.PointerEvent<HTMLDivElement>) {
    ...
    FACE_PRESETS.forEach((preset, index) => {
      const dx = prevX - preset.x;
      const dy = normalizeAngle(prevY - normalizeAngle(preset.y));
    ...
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <div className="mx-auto flex min-h-screen max-w-6xl flex-col px-4 pb-10 pt-4 sm:px-8 sm:pt-6">
        ...
        <header className="mb-6 flex items-center justify-between gap-3">
          ...
          >
            <span className="sr-only">Reteur</span>
                ...
                <div className="mx-auto mt-2 flex flex-col items-center">
                  ...
                  <div className="mb-3 text-center">
                    ...
                    {(() => {
                      ...
                      } else if (rawDescription && rawDescription.toLowerCase() !== faceNumberLabel.toLowerCase()) {
                      ...
                      return (
                        <>
                          ...
                          {displayTitle && (
                            <p className="mt-1 text-[13px] text-slate-600">{displayTitle}</p>
                          )}
                        </>
                      );
                    })()}
                      ...
                      {(() => {
                        ...
                        return facesForCube.map((face, index) => {
                          ...
                          const mergedFace: PreviewFace =
                            persisted?.needles
                              ? { ...face, needles: { ...(face.needles ?? {}), ...(persisted.needles ?? {}) } }
                              : face;
                          ...
                          return (
                            ...
                            >
                              {/* ─────────────────────────────────────────────
                                  FACE AVANT
                                  ⚠️ FIX MAC : les boutons ⤢ et ↗︎ sont sortis
                                  du div overflow-hidden et placés ici dans le
                                  wrapper [transform-style:preserve-3d] avec
                                  translateZ pour rester cliquables sur Mac.
                              ───────────────────────────────────────────── */}

                              {/* Contenu de la face (sans les boutons) */}
                              ...
                              {/* ✅ FIX MAC — Bouton ⤢ plein écran (bas gauche)
                                  Sorti du overflow-hidden → zone de hit réelle
                                  translateZ(1px) le projette devant la face */}
                              <button
                                type="button"
                                onClick={(e) => {
                                  e.stopPropagation();
                                ...
                                className="absolute left-3 bottom-3 z-[60] inline-flex h-10 w-10 min-h-[44px] min-w-[44px] -translate-x-0 items-center justify-center rounded-full border border-white/40 bg-white/90 text-xs text-slate-900 shadow-sm backdrop-blur [backface-visibility:hidden] hover:border-white hover:bg-white"
                              >
                                <span aria-hidden>⤢</span>
                              ...
                              {/* ✅ FIX MAC — Bouton ↗︎ flip/segments (bas droit)
                                  Sorti du overflow-hidden → zone de hit réelle
                                  translateZ(1px) le projette devant la face */}
                              <button
                                type="button"
                                onClick={(e) => {
                                  ...
                                  const firstId =
                                    (segs[0] as any)?.id ??
                                    (segs.length > 0 ? (segs[0] as any).id ?? 0 : null);
                                  setOpenedSegmentId(firstId ?? null);
                                }}
                                style={{ transform: "translateZ(1px)" }}
                                className="absolute right-3 bottom-3 z-[60] inline-flex h-10 w-10 min-h-[44px] min-w-[44px] items-center justify-center rounded-full border border-white/40 bg-white/90 text-xs text-slate-900 shadow-sm backdrop-blur [backface-visibility:hidden] hover:border-white hover:bg-white"
                              >
                                <span className="text-xs" aria-hidden>↗︎</span>
                                <span className="sr-only">
                              ...
                              <div className="absolute inset-0 rounded-none border border-slate-200 bg-slate-900/95 text-xs shadow-xl shadow-slate-900/30 [backface-visibility:hidden] [transform:rotateY(180deg)]">
                                <div className="relative flex h-full w-full items-center justify-center">
                                  ...
                                  {/* ✅ FIX MAC — Bouton ↺ retour (bas droit face arrière)
                                      Même fix : sorti du overflow-hidden, translateZ(1px) */}
                                  <button
                                    type="button"
                                    onClick={(e) => {
                                      e.stopPropagation();
                                    ...
                                    className="absolute right-3 bottom-3 z-[60] inline-flex h-10 w-10 min-h-[44px] min-w-[44px] items-center justify-center rounded-full border border-slate-300 bg-white/90 text-xs text-slate-700 shadow-sm backdrop-blur [backface-visibility:hidden] hover:border-slate-400 hover:bg-white"
                                  >
                                    <span aria-hidden>↺</span>
                                    <span className="sr-only">
              ...
              <section className="mt-6 w-full max-w-3xl self-center rounded-2xl border border-slate-200 bg-white/80 p-4 shadow-sm backdrop-blur-sm">
                ...
                <div className="mb-3 flex flex-wrap gap-2">
                  {detailSegments.map((seg, index) => {
                    ...
                    const label = (seg.title as string | undefined)?.trim() || `Segment ${index + 1}`;
                    const selected = openedSegmentId === id || activeDetailIndex === index;
                    return (
                      <button
                        key={id}
                        type="button"
                  ...
                  <div className="mt-2 space-y-3 rounded-2xl border border-slate-200 bg-white/95 p-3">
                    ...
                    <div className="rounded-2xl border border-slate-200 bg-white/80 p-3">
                        ...
                        <div className="relative mx-auto aspect-[4/5] w-full max-w-xl">
                          <div className="grid h-full w-full grid-cols-2">
                            <div className="relative h-full w-full">
                              {leftHasMedia && editorSelectedSegment.mediaUrl ? (
                                <>
                                  <LazyMediaSlot
                                    src={editorSelectedSegment.mediaUrl}
                                    kind={editorSelectedSegment.mediaType === "video" ? "video" : "photo"}
                                    alt="Avant"
                                    ...
                                    <button
                                      ...
                                      onClick={() =>
                                        setFullScreenSegmentMedia({
                                          url: editorSelectedSegment.mediaUrl as string,
                                          type: editorSelectedSegment.mediaType as MediaKind,
                                          label: editorSelectedSegment.label ?? "Avant",
                                        })
                                      }
                                      className="absolute left-3 bottom-3 z-10 inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/40 bg-white/90 text-xs text-slate-900 shadow-sm backdrop-blur hover:border-white hover:bg-white"
                                    >
                                      <span aria-hidden>⤢</span>
                                      <span className="sr-only">Afficher le média Avant en plein écran</span>
                                    </button>
                                  )}
                                </>
                              ) : (
                            ...
                            <div className="relative h-full w-full">
                              {rightHasMedia && editorOppositeSegment?.mediaUrl ? (
                                <>
                                  <LazyMediaSlot
                                    src={editorOppositeSegment.mediaUrl}
                                    kind={editorOppositeSegment.mediaType === "video" ? "video" : "photo"}
                                    alt="Après"
                                    className="absolute inset-0 h-full w-full object-cover"
                                  />
                                  {editorOppositeSegment.mediaType && (
                                    <button
                                      ...
                                      onClick={() =>
                                        setFullScreenSegmentMedia({
                                          url: editorOppositeSegment.mediaUrl as string,
                                          type: editorOppositeSegment.mediaType as MediaKind,
                                          label: editorOppositeSegment.label ?? "Après",
                                        })
                                      }
                                      className="absolute left-3 bottom-3 z-10 inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/40 bg-white/90 text-xs text-slate-900 shadow-sm backdrop-blur hover:border-white hover:bg-white"
                                    >
                                      <span aria-hidden>⤢</span>
                                      <span className="sr-only">Afficher le média Après en plein écran</span>
                                    </button>
                                  )}
                                </>
                              ) : (
                          ...
                          <div className="pointer-events-none absolute left-1/2 top-1/2 z-20 flex h-20 w-20 -translate-x-1/2 -translate-y-1/2 items-center justify-center rounded-full border border-white/90 shadow-sm">
                            {creatorAvatar ? (
                              ...
                              <img src={creatorAvatar} alt={creatorName} className="h-[72px] w-[72px] rounded-full object-cover" />
                            ) : (
                              <span className="text-base font-semibold text-white">{creatorInitials}</span>
                            )}
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="mt-3 grid gap-3 text-[11px] text-slate-600 md:grid-cols-2">
                        <div>
                          <p className="font-semibold text-slate-700">Segment {editorSelectedSegment.id} — Avant</p>
                          <p className="mt-0.5">{editorSelectedSegment.label || "Diagnostic / observation"}</p>
                        </div>
                        <div>
                          <p className="font-semibold text-slate-700">Segment {editorOppositeSegment.id} — Après</p>
                          <p className="mt-0.5">{editorOppositeSegment.label || "Préparation / sectionnement"}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid gap-3 md:grid-cols-2">
                      {editorSelectedSegment.notes && (
                        <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-3">
                          <p className="mb-1 text-[11px] font-semibold text-slate-700">Notes — Avant</p>
                          <p className="whitespace-pre-line text-[13px] text-slate-700">{editorSelectedSegment.notes}</p>
                        </div>
                      )}
                      {editorOppositeSegment?.notes && (
                        <div className="rounded-2xl border border-slate-100 bg-slate-50/70 p-3">
                          <p className="mb-1 text-[11px] font-semibold text-slate-700">Notes — Après</p>
                          <p className="whitespace-pre-line text-[13px] text-slate-700">{editorOppositeSegment.notes}</p>
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <>
                    <div className="rounded-2xl border border-slate-200 bg-white/80 p-3">
                      <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
                        <div className="relative mx-auto aspect-[4/5] w-full max-w-xl">
                          {(() => {
                            const mediaList = (activeDetailSegment?.media as any[]) ?? [];
                            if (!mediaList.length) {
                              return (
                            ...
                            const type = (media?.type as MediaKind | undefined) ?? "photo";
                            if (!url) {
                              return (
                                <div className="flex h-full w-full items-center justify-center text-xs text-slate-500">
                            ...
                            if (type === "file") {
                              ...
                              return (
                                <div className="flex h-full w-full flex-col items-center justify-center gap-2 text-xs text-slate-600">
                                  <div className="rounded-full border border-slate-300 bg-white/70 px-3 py-1">{filename}</div>
                          ...
                          {(() => {
                            ...
                            return (
                              <button
                                type="button"
                                onClick={() => setFullScreenSegmentMedia({ url, type, label: activeDetailSegment?.title ?? "" })}
                                className="absolute left-3 bottom-3 z-10 inline-flex h-8 w-8 items-center justify-center rounded-full border border-white/40 bg-white/90 text-xs text-slate-900 shadow-sm backdrop-blur hover:border-white hover:bg-white"
                              >
                                <span aria-hidden>⤢</span>
                                <span className="sr-only">Afficher ce média en plein écran</span>
                              </button>
                            );
                          })()}
                        </div>
                    ...
                    <div className="mt-3 space-y-1">
                      {segmentTitle && (
                        <p className="text-[11px] font-medium uppercase tracking-wide text-slate-500">{segmentTitle}</p>
                      )}
                      <p className="whitespace-pre-line text-[13px] text-slate-700">
                        {segmentNotes || "Pas de notes pédagogiques, tout est dit dans le titre."}
                      </p>
                    </div>
                  </>
                )}
            ...
            {(() => {
              ...
              return (
                <>
                  {imgUrl ? (
                    // eslint-disable-next-line @next/next/no-img-element
                    <img src={imgUrl} alt={label} className="h-[60vh] w-full bg-slate-950 object-contain" />
                  ) : (
                    <div className="flex h-[60vh] w-full flex-col items-center justify-center bg-gradient-to-br from-slate-800 via-slate-900 to-slate-950">
                      <p className="text-[11px] font-medium uppercase tracking-[0.3em] text-slate-300">Face {fullScreenFaceIndex + 1}</p>
                      <p className="mt-2 max-w-[70%] text-center text-base font-semibold text-slate-50">{label}</p>
                    </div>
                  )}
                  <div className="border-t border-slate-800 bg-slate-950/95 px-4 py-3 text-xs text-slate-100">
                    <p className="text-[10px] font-medium uppercase tracking-[0.24em] text-slate-400">Face {fullScreenFaceIndex + 1}</p>
                    <p className="mt-1 text-sm font-semibold text-slate-50">{label}</p>
                    {face.notes && face.notes.trim().length > 0 && (
                      <p className="mt-1 text-[11px] text-slate-300">{face.notes}</p>
                    )}
                  </div>
                </>
              );
      ...
      {/* 🔍 Overlay plein écran segment média */}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 backdrop-blur-sm">
          <div className="relative mx-4 w-full max-w-[520px] overflow-hidden rounded-3xl bg-slate-950 shadow-2xl">
            ...
            {(() => {
              ...
              if (!url) return (
                <div className="flex h-[60vh] w-full items-center justify-center text-xs text-slate-100">Média non disponible.</div>
              );
              if (type === "video") return (
                <>
                  {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
                  <video src={url} controls autoPlay className="h-[60vh] w-full bg-slate-950 object-contain" />
                  {label && (
                    <div className="border-t border-slate-800 bg-slate-950/95 px-4 py-3 text-xs text-slate-100">
                      <p className="text-[10px] font-medium uppercase tracking-[0.24em] text-slate-400">Segment</p>
                      <p className="mt-1 text-sm font-semibold text-slate-50">{label}</p>
                    </div>
                  )}
                </>
              ...
              if (type === "photo") return (
                <>
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img src={url} alt={label || "Média"} className="h-[60vh] w-full bg-slate-950 object-contain" />
                  {label && (
                    <div className="border-t border-slate-800 bg-slate-950/95 px-4 py-3 text-xs text-slate-100">
                      <p className="text-[10px] font-medium uppercase tracking-[0.24em] text-slate-400">Segment</p>
                      <p className="mt-1 text-sm font-semibold text-slate-50">{label}</p>
              ...
              return (
                <div className="flex h-[60vh] w-full flex-col items-center justify-center gap-2 text-xs text-slate-100">
                  ...
                  {label && <p className="text-[11px] text-slate-300">{label}</p>}
                </div>
              );
            })()}
          </div>
