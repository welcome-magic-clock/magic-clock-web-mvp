...
export async function POST(req: Request) {
  try {
    ...
    const { data: profile, error: profileError } = await supabaseAdmin
      ...
      .eq("id", user.id)             // UUID → profiles.id
      .single();
    ...
    const {
      ...
      studio, display, progress, payload, gatingMode, work,
    } = body as any;

    // Titre
    const rawTitle = title ?? studio?.title ?? work?.title ?? payload?.title ?? "Magic Clock";
    const finalTitle = String(rawTitle).trim();
    if (!finalTitle) return NextResponse.json({ ok: false, error: "Missing title" }, { status: 400 });
    ...
    // Hashtags
    const finalHashtags: string[] = Array.isArray(hashtags) ? hashtags
    ...
    // PPV
    const finalPpvPrice = finalMode === "PPV"
      ? (ppvPrice ?? studio?.ppvPrice ?? work?.ppvPrice ?? payload?.ppvPrice ?? null)
      : null;

    if (finalMode === "PPV" && (!finalPpvPrice || finalPpvPrice <= 0)) {
      return NextResponse.json({ ok: false, error: "Prix PPV obligatoire > 0" }, { status: 400 });
    }
    ...
    const finalStudio = {
      ...
      beforeCoverTime: beforeCoverTime ?? studio?.beforeCoverTime ?? work?.studio?.beforeCoverTime ?? null,
      afterCoverTime: afterCoverTime ?? studio?.afterCoverTime ?? work?.studio?.afterCoverTime ?? null,
    };
    ...
    // ✅ INSERT — user_id UUID + handle public
