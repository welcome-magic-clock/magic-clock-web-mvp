...
// ✅ v4.1 — Bloc créateur → anneau gradient rotatif MCAvatar canonique
...
const STAR_GRAD: React.CSSProperties = { background: GRAD_BG, WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" };

interface ClockData {
  id: string; slug: string; title: string;
  gatingMode: "FREE" | "SUB" | "PPV"; ppvPrice: number | null;
  creatorHandle: string; creatorName: string; creatorAvatar: string | null;
  creatorBio: string | null; creatorFollowers: number;
  beforeUrl: string | null; afterUrl: string | null; thumbnailUrl: string | null;
  ratingAvg: number | null; ratingCount: number;
  creatorRatingAvg: number | null; viewsCount: number; likesCount: number;
  hashtags: string[];
}
...
function StarRow({ value, count, size = 11 }: { value?: number; count?: number; size?: number }) {
  ...
  return (
    <span className="inline-flex items-center gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <span key={s} style={{
          fontSize: size, lineHeight: 1, fontWeight: 900,
          ...(s <= filled
      ...
      {value !== undefined && (
        <>
          <span className="ml-0.5 font-bold" style={{ fontSize: size - 1, ...STAR_GRAD }}>{value.toFixed(1)}</span>
          {count !== undefined && count > 0 && (
            <span className="ml-0.5 text-slate-300" style={{ fontSize: size - 2 }}>({count})</span>
          )}
        </>
      )}
    </span>
...
export default function MagicClockDetailClient({ clock }: { clock: ClockData }) {
  ...
  useEffect(() => {
    fetch(`/api/magic-clocks/${clock.id}/view`, { method: "POST" })
      .then(() => setViewsCount(v => v + 1)).catch(() => {});
  ...
  async function handleLike() {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        if (data.liked === wasLiked) setLikesCount(c => wasLiked ? c + 1 : Math.max(0, c - 1));
      }
    } catch {
      setLiked(wasLiked);
      setLikesCount(c => wasLiked ? c + 1 : Math.max(0, c - 1));
  ...
  const btnCfg = clock.gatingMode === "FREE"
  ...
  async function handleCTA() {
    ...
    try {
      if (clock.gatingMode === "FREE") {
        ...
        if (res.ok) {
          router.push(`/access/result?status=ok&contentId=${encodeURIComponent(clock.id)}&creator=${encodeURIComponent("@" + clock.creatorHandle)}&slug=${clock.slug}&title=${encodeURIComponent(clock.title)}&thumb=${encodeURIComponent(clock.thumbnailUrl ?? "")}`);
        } else { router.push("/access/result?status=error"); }
      } else {
        const res = await fetch("/api/payments/checkout", {
          ...
          body: JSON.stringify({
            ...
            currency: "chf", creatorId: clock.creatorHandle, buyerCountryCode: "CH",
        ...
        if (data.status === "mock" || !data.clientSecret) {
          router.push(`/access/result?status=ok&contentId=${encodeURIComponent(clock.id)}&creator=${encodeURIComponent("@" + clock.creatorHandle)}`);
        } else {
    ...
    } catch { router.push("/access/result?status=error"); }
    ...
    <main className="mx-auto max-w-lg pb-32 pt-0 min-h-screen bg-[#f8fafc]">
      ...
      <div className="relative w-full overflow-hidden bg-slate-200" style={{ aspectRatio: "4/5" }}>
        {displayImg ? (
          ...
          <img src={displayImg} alt={clock.title} className="h-full w-full object-cover transition-opacity duration-300" />
        ) : (
        ...
        {clock.beforeUrl && clock.afterUrl && (
          <div className="absolute bottom-4 left-1/2 -translate-x-1/2 flex rounded-full overflow-hidden"
            ...
            <button type="button" onClick={() => setShowBefore(true)}
              ...
              style={{ background: showBefore ? "white" : "rgba(255,255,255,.25)", color: showBefore ? "#1e293b" : "white" }}>Avant</button>
            <button type="button" onClick={() => setShowBefore(false)}
              className="px-4 py-1.5 text-[11px] font-bold transition-colors"
              style={{ background: !showBefore ? "white" : "rgba(255,255,255,.25)", color: !showBefore ? "#1e293b" : "white" }}>Après</button>
          </div>
        )}
      ...
      <div className="px-4 pt-4 space-y-4">
        <div>
          ...
          {clock.hashtags.length > 0 && (
            <div className="mt-1.5 flex flex-wrap gap-1.5">
              {clock.hashtags.map(tag => <span key={tag} className="text-[11px] font-medium text-slate-400">{tag}</span>)}
            </div>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 text-[11px] text-slate-400">
          <StarRow value={clock.ratingAvg ?? undefined} count={clock.ratingCount} size={11} />
          <span className="flex items-center gap-1"><Eye className="h-3 w-3" />{formatN(viewsCount)}</span>
        ...
        {/* ✅ Carte créateur — MCAvatar rotatif */}
        <Link
        ...
        >
          ...
          <div className="flex-1 min-w-0">
            ...
            {clock.creatorBio && <p className="mt-0.5 text-[11px] text-slate-500 line-clamp-1">{clock.creatorBio}</p>}
          </div>

          <div className="flex flex-col items-end gap-1 flex-shrink-0">
            <StarRow value={clock.creatorRatingAvg ?? undefined} size={10} />
        ...
        <div className="mx-auto max-w-lg">
          <button type="button" onClick={handleCTA} disabled={loading}
            ...
            {loading
              ...
              : <><btnCfg.Icon className="h-4 w-4" />{btnCfg.label}</>
            }
          </button>
        </div>
      </div>
