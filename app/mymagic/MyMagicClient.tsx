...
// ✅ REDESIGN v2 — Magic Clock Design System + totalFollowers dans le header
"use client";
...
// Carte créateur
// ─────────────────────────────────────────────────────────────
function CreatorCard({ clock, avatarUrl, isHighlighted, cardRef }: {
  ...
  return (
    <article ref={cardRef} className={`rounded-2xl border bg-white shadow-sm transition-all overflow-hidden ${isHighlighted ? "border-violet-400 ring-2 ring-violet-200 ring-offset-2" : "border-slate-200"}`}>
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-slate-100">
        ...
        <Link href={displayHref(clock.id, clock.slug)} prefetch={false} className="absolute right-2.5 top-2.5 z-10 flex h-8 w-8 items-center justify-center rounded-full bg-black/60 text-white shadow backdrop-blur-sm hover:bg-black/80 transition-colors">
          <ArrowUpRight className="h-4 w-4" />
        </Link>
        <div className="absolute bottom-2.5 left-2.5 z-10">
          {mode === "FREE"
            ? <span className="rounded-md bg-emerald-500/90 px-2 py-0.5 text-[10px] font-bold text-white">FREE</span>
            : <span className="rounded-md bg-violet-500/90 px-2 py-0.5 text-[10px] font-bold text-white">{mode === "SUB" ? "Abonnement" : "PPV"}</span>
          }
        </div>
      </div>
      <div className="p-3">
        <p className="font-semibold text-slate-900 text-sm leading-tight">{title}</p>
        {hashtags.length > 0 && <p className="mt-1 text-[11px] text-violet-500 truncate">{hashtags.slice(0, 3).join(" ")}</p>}
        <Link href={displayHref(clock.id, clock.slug)} prefetch={false} className="mt-1.5 inline-flex items-center gap-1 text-[11px] font-semibold mc-text-gradient hover:opacity-80">
...
// Carte acquise
// ─────────────────────────────────────────────────────────────
function AcquiredCard({ item, isHighlighted, cardRef }: {
  ...
  return (
    <article ref={cardRef} className={`rounded-2xl border bg-white shadow-sm overflow-hidden transition-all ${isHighlighted ? "border-violet-400 ring-2 ring-violet-200 ring-offset-2" : "border-slate-200"}`}>
      <div className="relative aspect-[4/5] w-full overflow-hidden bg-slate-100">
        <div className="grid h-full w-full grid-cols-2">
          <StudioMediaSlot src={beforeUrl} alt={`${title} - Avant`} coverTime={studio.beforeCoverTime} />
          <StudioMediaSlot src={afterUrl}  alt={`${title} - Après`} coverTime={studio.afterCoverTime} />
...
function LevelCard({ publishedCount }: { publishedCount: number }) {
  ...
  const nextNames  = ["Horloger Certifié", "Maître Horloger", "Grand Maître"];
  ...
  return (
    <div className="mx-4 mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm cursor-pointer hover:border-violet-200 hover:shadow-md transition-all">
      <div className="flex items-center gap-3">
        <div className="flex h-11 w-11 flex-shrink-0 items-center justify-center rounded-xl border border-slate-100 bg-slate-50 text-xl">⏱</div>
        <div className="flex-1 min-w-0">
          <p className="text-[13px] font-bold mc-text-gradient">{levelNames[level - 1]} · Niveau {level}</p>
          <p className="text-[11px] text-slate-400 mt-0.5 truncate">{Math.round(progress)}% vers {nextNames[level - 1]}</p>
...
export function MyMagicClient({ initialPublished = [], initialAcquired = [] }: MyMagicClientProps) {
  ...
  // ── Total followers sociaux (mis à jour depuis ProfileSection ou au chargement) ──
  ...
  useEffect(() => {
    ...
    sb.from("profiles")
      ...
      .then(({ data }) => {
        ...
        // Calcul total abonnés toutes plateformes
        const total = [
        ...
        ].reduce((sum: number, v: any) => sum + (v ?? 0), 0);
      ...
      <div className="px-4 mb-3">
        ...
        <div className="flex overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm mb-4">
          {[
            ...
            {
              value: totalSocialFollowers > 0 ? formatFollowersShort(totalSocialFollowers) : "0",
              label: "Followers",
            },
            { value: "0", label: "Vues" },
            { value: "—", label: "Note" },
        ...
        <div className="flex gap-2.5 mb-4">
          ...
          <button type="button" className="flex items-center gap-2 rounded-full border border-slate-200 bg-white px-5 py-3 text-[13.5px] font-semibold text-slate-700 shadow-sm hover:border-slate-300 hover:bg-slate-50 transition-colors">
            ...
            <div>
              ...
              <div className="flex gap-3 overflow-x-auto pb-2 -mx-4 px-4 scrollbar-none">
                {initialPublished.map((clock) => {
                  ...
                  const mode      = clock.gating_mode ?? "FREE";
                  return (
                    <Link key={clock.id} href={displayHref(clock.id, clock.slug)}
                      ...
                      <div className="p-2.5">
                        ...
                        <div className="mt-1 flex items-center gap-1.5 text-[10px] text-slate-400">
                          <span className={`rounded px-1.5 py-0.5 font-bold text-[9px] ${mode === "FREE" ? "bg-emerald-100 text-emerald-700" : "bg-violet-100 text-violet-700"}`}>{mode}</span>
                        </div>
                      </div>
                    </Link>
                  );
            ...
            {initialPublished.length === 0 ? (
              ...
              <div className="grid grid-cols-2 gap-3">
                {initialPublished[0] && (() => {
                  ...
                  const mode      = clock.gating_mode ?? "FREE";
                  return (
                    <Link href={displayHref(clock.id, clock.slug)}
                ...
                {initialPublished.slice(1).map((clock) => (
                  <CreatorCard key={clock.id} clock={clock} avatarUrl={profileAvatarUrl}
                ...
                <Link href="/studio" className="col-span-2 rounded-2xl border border-dashed border-violet-200 bg-gradient-to-br from-violet-50/60 to-pink-50/40 p-5 flex flex-col items-center gap-2 text-center hover:border-violet-300 hover:from-violet-50 transition-colors">
          ...
          {initialAcquired.length > 0 && (
            <div>
              <h3 className="mb-3 flex items-center gap-2 text-[11px] font-bold uppercase tracking-wider text-slate-400 after:flex-1 after:h-px after:bg-slate-100 after:content-['']">
                <BookOpen className="h-3 w-3" /> Bibliothèque débloquée
              </h3>
              <div className="grid grid-cols-2 gap-3">
                {initialAcquired.map((item) => (
                  <AcquiredCard key={item.magic_clock_id} item={item}
                    isHighlighted={!!openParam && (item.id === openParam || item.slug === openParam || item.magic_clock_id === openParam)}
                    cardRef={(el) => { acquiredCardRefs.current[item.id] = el; acquiredCardRefs.current[item.magic_clock_id] = el; if (item.slug) acquiredCardRefs.current[item.slug] = el; }}
                  />
                ))}
              </div>
            </div>
        ...
        <section className="px-4 pt-5">
          <ProfileSection
            ...
            onProfileUpdated={(u) => {
              ...
              // ✅ Mise à jour du total followers en temps réel
        ...
        <section className="px-4 pt-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              { value: totalSocialFollowers > 0 ? formatFollowersShort(totalSocialFollowers) : "0", label: "Abonnés totaux" },
