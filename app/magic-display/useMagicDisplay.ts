...
// ✅ v2.3 — Fix PPV : ppvPrice au niveau racine du body fetch
"use client";
...
export function useMagicDisplay(searchParams: URLSearchParams) {
  ...
  useEffect(() => {
    try {
      ...
      const beforeIsCorrupt = payload.before?.url ? !isCdnUrl(payload.before.url) : false;
      const afterIsCorrupt = payload.after?.url ? !isCdnUrl(payload.after.url) : false;
      ...
      if (payload.before?.url && isCdnUrl(payload.before.url)) setStudioBeforeUrl(payload.before.url);
      if (typeof payload.before?.coverTime === "number") setStudioBeforeCover(payload.before.coverTime);
      if (payload.before?.thumbnailUrl && isCdnUrl(payload.before.thumbnailUrl)) setStudioBeforeThumb(payload.before.thumbnailUrl);
      if (payload.after?.url && isCdnUrl(payload.after.url)) setStudioAfterUrl(payload.after.url);
      if (typeof payload.after?.coverTime === "number") setStudioAfterCover(payload.after.coverTime);
      if (payload.after?.thumbnailUrl && isCdnUrl(payload.after.thumbnailUrl)) setStudioAfterThumb(payload.after.thumbnailUrl);
      if (payload.title) setBridgeTitle(payload.title);
      if (payload.mode) setBridgeMode(payload.mode as PublishMode);
      if (typeof payload.ppvPrice === "number") setBridgePpvPrice(payload.ppvPrice);
      if (Array.isArray(payload.hashtags)) setBridgeHashtags(payload.hashtags);
  ...
  const { displayPart, completedFaces, partialFaces } = computeMagicClockPublishProgress({
    studioCompleted,
  ...
  const beforePreview =
    studioBeforeThumb ?? studioAfterThumb ?? studioBeforeUrl ?? studioAfterUrl ?? "/images/magic-clock-bear/before.jpg";
  const afterPreview =
    studioAfterThumb ?? studioBeforeThumb ?? studioAfterUrl ?? studioBeforeUrl ?? "/images/magic-clock-bear/after.jpg";
  ...
  function handleFaceEditorChange(payload: FaceDetailsPayload) {
    ...
    const universalContentCompleted =
      ...
      faceSegments
        ...
        .every((s) => (s.notes?.trim().length ?? 0) > 0 || (s.media?.length ?? 0) > 0);
      ...
      try {
        ...
        const res = await fetch("/api/magic-clocks/create", {
          ...
          body: JSON.stringify({
            ...
            // ✅ FIX v2.3 : ppvPrice au niveau racine pour que l'API le trouve directement
            ppvPrice: effectiveMode === "PPV" ? (effectivePpvPrice ?? null) : null,
            work: workPayload,
          }),
        });
        ...
        if (res.ok) {
          ...
          const errMsg =
            json?.error ?? `Erreur serveur (${res.status}). Ton Magic Clock n'a pas été sauvegardé. Réessaie.`;
  ...
  return {
    ...
    handleSelectedDescriptionChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      ...
      setSegments((prev) =>
        prev.map((seg) => (seg.id === selectedId ? { ...seg, description: value } : seg))
      );
    },
    handleSelectedNotesChange: (e: React.ChangeEvent<HTMLTextAreaElement>) => {
      setSegments((prev) =>
        prev.map((seg) => (seg.id === selectedId ? { ...seg, notes: e.target.value } : seg))
      );
    },
  };
}
