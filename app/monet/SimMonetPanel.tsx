...
// ✅ v4.3 — Chiffres PPV gradient unifié + carte Commission mention abonnement + palette harmonisée
"use client";
...
export function SimMonetPanel({ creator }: Props) {
  ...
  // ── Gradient partagé sur TOUS les chiffres clés (Abo, PPV, résumé)
  const gradientStyle = {
  ...
  // ── Badge tier PPV : gradient doux harmonisé (plus sky-blue agressif)
  const tierBadgeStyle = {
    background: "linear-gradient(135deg,rgba(123,75,245,.08),rgba(196,75,218,.08))",
    ...
    <div className="space-y-0 pb-8">
          ...
          <div className="flex items-center gap-2.5">
            <div className="flex h-8 w-8 items-center justify-center rounded-[10px] bg-violet-100">
              <svg
                ...
                stroke="#7B4BF5" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"
              >
        ...
        <div className="mb-3">
          ...
          <div className="mt-2 flex items-center gap-2 text-[11px] text-slate-400">
            ...
            <input
              ...
              onChange={(e) =>
                setSimFollowers(clamp(Number(e.target.value.replace(/^0+(?=\d)/, "")) || 0, 0, 1000000000))
              }
        ...
        <div className="flex items-center gap-2 rounded-xl border border-slate-100 bg-slate-50 px-3 py-2">
          ...
          <div>
            ...
            <span className="ml-1.5 text-[10px] text-slate-400">
              Basé sur {simLikes.toLocaleString("fr-CH")} likes
            </span>
          </div>
      ...
      <div className="mx-4 mt-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            <div className="mb-1 flex justify-between text-[12px]">
              ...
              <span className="font-black" style={gradientStyle}>
                {simAboPrice.toFixed(2)} CHF
              </span>
            </div>
            <input
        ...
        <p className="mt-2 text-[11px] text-slate-400">
          ≈ {Math.round(aboSubs).toLocaleString("fr-CH")} abonnés ·{" "}
          {formatMoney(grossAbos)} brut / mois
        </p>
      </div>
      ...
      <div className="mx-4 mt-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="mb-3 flex items-center justify-between">
          ...
          {/* Badge tier : gradient doux harmonisé (plus sky agressif) */}
          <div
          ...
          <div>
            ...
            <div className="mt-1 flex items-center gap-1 text-[11px] text-slate-400">
              ...
              <input
                ...
                onFocus={(e) => e.target.select()}
                onChange={(e) =>
                  setSimPpvPerBuyer(clamp(Number(e.target.value.replace(/^0+(?=\d)/, "")) || 0, 0, 100000000))
                }
                className="w-16 rounded-xl border border-slate-200 bg-slate-50 px-2 py-1 text-right text-[11px] font-bold text-slate-700 outline-none focus:border-violet-400"
              />
            </div>
        ...
        <p className="mt-2 text-[11px] text-slate-400">
          ≈ {Math.round(ppvBuyers).toLocaleString("fr-CH")} acheteurs ·{" "}
          {formatMoney(grossPpv)} brut / mois
        </p>
      </div>
      ...
      <div className="mx-4 mt-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
        ...
        <div
          ...
          style={{
            background: "linear-gradient(135deg,rgba(123,75,245,.06),rgba(196,75,218,.06))",
            border: "1px solid rgba(123,75,245,.12)",
          }}
        >
          <div>
            <p className="text-[13px] font-bold text-slate-800">Tu gardes estimé</p>
            <p className="text-[10px] text-slate-400">
              Après TVA + commission · versé le 15
            </p>
          </div>
          <p className="text-[24px] font-black leading-tight" style={gradientStyle}>
      ...
      <div className="mx-4 mt-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
              ...
              <div
                ...
                style={
                  isActive
                    ? {
                        background:
                          "linear-gradient(to right,rgba(123,75,245,.06),rgba(196,75,218,.06))",
                        border: "1px solid rgba(123,75,245,.18)",
                        color: "#7B4BF5",
                      }
                    : {}
        ...
        {/* Barre de gradient adaptée au violet */}
        <div className="mt-3 h-1.5 overflow-hidden rounded-full bg-slate-200">
          <div
            className="h-full rounded-full"
            style={{
              background:
                "linear-gradient(to right,#a78bfa,#7B4BF5,#C44BDA,#F54B8F)",
            }}
          />
        </div>
