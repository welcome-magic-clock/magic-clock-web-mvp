...
// ✅ REDESIGN v2.1 — 12 réseaux sociaux avec vrais logos + handles éditables + followers
"use client";

import { useEffect, useRef, useState } from "react";
import {
  UserRound, FileText, Camera,
  Loader2, CheckCircle2, AlertCircle, ArrowRight, X,
} from "lucide-react";
...
// Formate les followers : 12400 → "12,4k" · 1200000 → "1,2M"
...
function SocialCard({
  ...
  return (
    <div className={`rounded-xl border transition-all duration-200 ${
      isLinked ? "border-violet-200 bg-violet-50/30" : "border-slate-100 bg-white"
    }`}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {isLinked ? (
            ...
            <p className="text-[10px] text-slate-400 leading-tight">Cliquer pour connecter</p>
          )}
        </div>

        {/* Indicateur + chevron */}
        <div className="flex items-center gap-2 flex-shrink-0">
          {isLinked && <span className="h-1.5 w-1.5 rounded-full bg-emerald-400" />}
...
export default function ProfileSection({
  ...
  const emptyState = () => ({ handle: "", followers: null as number | null });
  const [socials, setSocials] = useState<Record<SocialKey, { handle: string; followers: number | null }>>({
  ...
  useEffect(() => {
    ...
    sb.from("profiles")
      .select(`
        ...
        social_tiktok, social_tiktok_followers,
        social_instagram, social_instagram_followers,
        social_youtube, social_youtube_followers,
        social_facebook, social_facebook_followers,
        social_linkedin, social_linkedin_followers,
        social_snapchat, social_snapchat_followers,
        social_x, social_x_followers,
        social_magic_clock, social_magic_clock_followers,
        social_pinterest, social_pinterest_followers,
        social_threads, social_threads_followers,
        social_bereal, social_bereal_followers,
        social_twitch, social_twitch_followers
      `)
      .eq("id", userId)
      .single()
      .then(({ data }) => {
        ...
        setSocials({
          magic_clock: { handle: data.social_magic_clock ?? "",  followers: data.social_magic_clock_followers ?? null },
          instagram:   { handle: data.social_instagram ?? "",    followers: data.social_instagram_followers ?? null },
          tiktok:      { handle: data.social_tiktok ?? "",       followers: data.social_tiktok_followers ?? null },
          youtube:     { handle: data.social_youtube ?? "",      followers: data.social_youtube_followers ?? null },
          facebook:    { handle: data.social_facebook ?? "",     followers: data.social_facebook_followers ?? null },
          linkedin:    { handle: data.social_linkedin ?? "",     followers: data.social_linkedin_followers ?? null },
          snapchat:    { handle: data.social_snapchat ?? "",     followers: data.social_snapchat_followers ?? null },
          x:           { handle: data.social_x ?? "",            followers: data.social_x_followers ?? null },
          pinterest:   { handle: data.social_pinterest ?? "",    followers: data.social_pinterest_followers ?? null },
          threads:     { handle: data.social_threads ?? "",      followers: data.social_threads_followers ?? null },
          bereal:      { handle: data.social_bereal ?? "",       followers: data.social_bereal_followers ?? null },
          twitch:      { handle: data.social_twitch ?? "",       followers: data.social_twitch_followers ?? null },
        });
      });
  }, [userId]);
  ...
  async function handleSaveProfile() {
    ...
    try {
      ...
      const { error: upsertErr } = await sb.from("profiles").upsert({
        ...
        social_magic_clock:           socials.magic_clock.handle || null,
        social_magic_clock_followers: socials.magic_clock.followers,
      ...
      onProfileUpdated?.({ handle: handle.trim().replace(/^@/, ""), display_name: displayName.trim(), bio: bio.trim(), avatar_url: finalAvatarUrl });
    ...
    <div className="space-y-3">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <p className="mt-3 text-[10px] text-slate-400 leading-relaxed">
          Tes abonnés cumulés sont visibles sur ton profil public <span className="font-semibold">Meet Me</span> — renforce ta crédibilité auprès de ton audience sur toutes les plateformes.
        </p>
      </div>
